...
let RolesGuard = RolesGuard_1 = class RolesGuard {
    ...
    canActivate(context) {
        ...
        if (!hasRole) {
            this.logger.warn(`⛔ Accès refusé: utilisateur avec rôle "${user.role}" tente d'accéder à une ressource nécessitant ${requiredRoles.join(' ou ')}`);
            throw new common_1.ForbiddenException(`Accès refusé. Rôle requis: ${requiredRoles.join(' ou ')}`);
        }
